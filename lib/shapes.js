...
class Shape {
    ...
    */
    setText(t){
        if(t.length > 3){
            console.log("Text length too long, remains unchanged");
            return false;
        }
        this.chars = t;
        return true;
    ...
    setTcolor(c){
        if(!colorCheck(c)){
            console.log(`Color [${c}] is bad.\nMust be common color name or 3/6 hex (led with #), remains unchanged`);
            return false;
        }
        this.tcolor = c;
        return true;
    ...
    // ... the first line and dependent on child classes to complete output following a super().
    render(){
        return `<svg version="1.1" width="300" height="200" xmlns="http://www.w3.org/2000/svg">`;
    }
    // Similarly, the line of code for text completes all output SVG files so this is called
    // ... in most child classes to finish the output.
    render2(){
